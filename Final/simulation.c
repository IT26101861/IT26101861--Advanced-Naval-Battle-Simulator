...
#define MIN_B_GAMMA 0.001
#define MAX_B_GAMMA 0.200
#define MIN_E_GAMMA 0.010
#define MAX_E_GAMMA 1.000
#define MIN_EFFECTIVE_IMPACT 0.000001

#define STAGE_PART_1 0
#define STAGE_PART_2_A 1
#define STAGE_PART_2_B 2
#define STAGE_PART_2_C 3
...
typedef struct
{
    ...
    int bShotFired;
    double bFireTime;
    int bFirstShotNumber;
    int bShotsPlanned;
    int bShotsFired;
    int bExpectedDestroyed;
    double bFlightTime;
    double bLastImpactTime;
    double bPlannedFinalImpactTime;
...
static int activeStage = STAGE_PART_1;
...
static double battleshipGammaValues[4];
static double escortGammaValues[5];
static int battleshipShotsFired = 0;
static double battleshipCurrentImpactPower = 1.0;
...
static void resetEscortStepData(void)
{
    ...
    for (i = 0; i < numberOfEscorts; i++)
    {
        ...
        escorts[i].bFirstShotNumber = 0;
        escorts[i].bShotsPlanned = 0;
        escorts[i].bShotsFired = 0;
        escorts[i].bExpectedDestroyed = 0;
        escorts[i].bFlightTime = 0.0;
        escorts[i].bLastImpactTime = -1.0;
        escorts[i].bPlannedFinalImpactTime = -1.0;
...
static void resetScenarioState(void)
{
    ...
    battleshipShotsFired = 0;
    battleshipCurrentImpactPower = 1.0;
    ...
    for (i = 0; i < numberOfEscorts; i++)
    {
        ...
        escorts[i].stepShotsFired = 0;
        escorts[i].damage = 0.0;
        escorts[i].currentImpactPower = escorts[i].type->impactPower;
    }
...
static void showConfiguration(void)
{
    ...
    printf("Shell Velocity      : %.2f - %.2f\n",
           battleshipMinVelocity,
           battleshipMaxVelocity);
    printf("B Fire Mode         : %s\n",
           activeStage == STAGE_PART_1 ? "Instant" : "Sequential");

    if (activeStage >= STAGE_PART_2_A)
    {
    ...
    printf("Escort Fire Mode    : %s\n",
           activeStage >= STAGE_PART_2_B ? "Continuous" : "Single shot");

    if (activeStage >= STAGE_PART_2_B)
    {
        printf("E Reload Times      : EA %.2f | EB %.2f | EC %.2f | ED %.2f | EE %.2f s\n",
               escortReloadTimes[0], escortReloadTimes[1],
               escortReloadTimes[2], escortReloadTimes[3],
               escortReloadTimes[4]);
    }

    if (activeStage == STAGE_PART_2_C)
    {
        printf("B Gamma             : %.4f\n",
               battleshipGammaValues[battleshipChoice - 1]);
        printf("E Gamma Values      : EA %.3f | EB %.3f | EC %.3f | ED %.3f | EE %.3f\n",
               escortGammaValues[0], escortGammaValues[1],
               escortGammaValues[2], escortGammaValues[3],
               escortGammaValues[4]);
    }
...
static void saveInitialConditions(const char *mode)
{
    ...
    fprintf(file, "Escort Fire Mode  : %s\n",
            activeStage >= STAGE_PART_2_B ? "Continuous" : "Single shot");

    if (activeStage == STAGE_PART_2_C)
    {
        fprintf(file, "B Gamma           : %.6f\n",
                battleshipGammaValues[battleshipChoice - 1]);
    }
    ...
    if (activeStage == STAGE_PART_2_C)
    {
        fprintf(file, "E Gamma Values    : EA %.6f | EB %.6f | EC %.6f | ED %.6f | EE %.6f\n",
                escortGammaValues[0], escortGammaValues[1],
                escortGammaValues[2], escortGammaValues[3],
                escortGammaValues[4]);
    }
    ...
    for (i = 0; i < numberOfEscorts; i++)
    {
        ...
        if (activeStage == STAGE_PART_2_C)
        {
            fprintf(file, "Gamma             : %.6f\n",
                    escortGammaValues[(int)(escorts[i].type - escortTypes)]);
        }
    }
...
static double battleshipImpactForShot(int shotNumber)
{
    if (activeStage != STAGE_PART_2_C)
        return 1.0;

    return exp(
        -battleshipGammaValues[battleshipChoice - 1] * shotNumber
    );
}

static double escortImpactForShot(int escortIndex, int shotNumber)
{
    int typeIndex = (int)(escorts[escortIndex].type - escortTypes);

    if (activeStage != STAGE_PART_2_C)
        return escorts[escortIndex].type->impactPower;

    return escorts[escortIndex].type->impactPower *
           exp(-escortGammaValues[typeIndex] * shotNumber);
}
...
static void scheduleBattleshipAttackOrder(void)
{
    ...
    int targetCount = 0;
    int plannedShotNumber = battleshipShotsFired;
    int plannedShots = 0;
    ...
    {
        int index = targets[i];
        double projectedDamage = escorts[index].damage;
        double flightTime = escorts[index].bImpactTime;

        escorts[index].bFlightTime = flightTime;
        ...
        {
            plannedShotNumber++;
            escorts[index].bFireTime =
                activeStage == STAGE_PART_1 ? 0.0 : nextFireTime;
            escorts[index].bFirstShotNumber = plannedShotNumber;
            escorts[index].bShotsPlanned = 1;
            escorts[index].bLastImpactTime = nextFireTime + flightTime;
            projectedDamage += 1.0;
            plannedShots++;

            if (activeStage != STAGE_PART_1)
                nextFireTime += battleshipReloadTime;
        }
        else
        {
            while (projectedDamage < DESTROYED_DAMAGE - 0.0000001 &&
                   plannedShots < MAX_IMPACT_RECORDS)
            {
                double impactPower;

                plannedShotNumber++;
                impactPower = battleshipImpactForShot(plannedShotNumber);

                if (impactPower < MIN_EFFECTIVE_IMPACT)
                    break;

                if (escorts[index].bShotsPlanned == 0)
                {
                    escorts[index].bFireTime = nextFireTime;
                    escorts[index].bFirstShotNumber = plannedShotNumber;
                }

                escorts[index].bShotsPlanned++;
                escorts[index].bLastImpactTime = nextFireTime + flightTime;
                projectedDamage += impactPower;
                plannedShots++;
                nextFireTime += battleshipReloadTime;
            }
        }

        escorts[index].bShotFired = escorts[index].bShotsPlanned > 0;
        escorts[index].bExpectedDestroyed =
            projectedDamage >= DESTROYED_DAMAGE - 0.0000001;
        escorts[index].bPlannedFinalImpactTime =
            escorts[index].bLastImpactTime;
        escorts[index].bImpactTime = escorts[index].bLastImpactTime;

        if (plannedShots >= MAX_IMPACT_RECORDS)
            break;
    }
}
...
{
    int highestFiredShotNumber = battleshipShotsFired;
    int i;
    ...
    *lastBImpactTime = 0.0;
    ...
    for (i = 0; i < numberOfEscorts; i++)
    {
        ...
        if (escorts[i].targetedByBattleship)
        {
            ...
            for (shot = 0; shot < escorts[i].bShotsPlanned; shot++)
            {
                int shotNumber = escorts[i].bFirstShotNumber + shot;
                double fireTime = escorts[i].bFireTime +
                                  shot * battleshipReloadTime;
                double impactTime = fireTime + escorts[i].bFlightTime;
                double impactPower;

                if (!battleshipAlive &&
                    fireTime > sinkTime + 0.000001)
                {
                    break;
                }

                impactPower = battleshipImpactForShot(shotNumber);
                escorts[i].bShotsFired++;
                escorts[i].damage += impactPower;

                if (shotNumber > highestFiredShotNumber)
                    highestFiredShotNumber = shotNumber;

                if (impactTime > *lastBImpactTime)
                    *lastBImpactTime = impactTime;

                escorts[i].bImpactTime = impactTime;

                if (escorts[i].damage >= DESTROYED_DAMAGE - 0.0000001)
                {
                    escorts[i].alive = 0;
            ...
            escorts[i].bShotFired = escorts[i].bShotsFired > 0;
        }
    }

    battleshipShotsFired = highestFiredShotNumber;

    if (battleshipShotsFired > 0)
    {
        battleshipCurrentImpactPower =
            battleshipImpactForShot(battleshipShotsFired);
    }
}
...
)
{
    ...
    *sinkTime = -1.0;
    ...
    for (i = 0; i < numberOfEscorts; i++)
        startingShots[i] = escorts[i].totalShotsFired;

    while (impactRecordCount < MAX_IMPACT_RECORDS)
    {
        int selected = -1;
        int selectedShot = 0;
        double earliest = DBL_MAX;
        double selectedImpactPower = 0.0;
        ...
        for (i = 0; i < numberOfEscorts; i++)
        {
            ...
            if (activeStage < STAGE_PART_2_B && nextShot[i] > 0)
                continue;

            typeIndex = (int)(escorts[i].type - escortTypes);
            launchTime = nextShot[i] * escortReloadTimes[typeIndex];
            shotNumber = startingShots[i] + nextShot[i] + 1;
            impactPower = escortImpactForShot(i, shotNumber);
            ...
            if (impactTime < earliest)
            {
                ...
                selectedImpactPower = impactPower;
            }
        }
        ...
        escorts[selected].totalShotsFired =
            startingShots[selected] + nextShot[selected];
        escorts[selected].currentImpactPower = selectedImpactPower;

        battleshipDamage += selectedImpactPower;
        ...
        if (impactRecordCount < MAX_IMPACT_RECORDS)
        {
            ...
            impactRecords[impactRecordCount].impactPower = selectedImpactPower;
...
static void printBattleshipAttackOrder(void)
{
    ...
    for (order = 1; order <= numberOfEscorts; order++)
    {
        ...
        for (i = 0; i < numberOfEscorts; i++)
        {
            ...
            {
                printf(
                    "%2d. E%03d (%s) | Planned shots %d | First fire %.2f s | Final impact %.4f s | %s%s\n",
                    order,
                    escorts[i].id,
                    escorts[i].type->notation,
                    escorts[i].bShotsPlanned,
                    escorts[i].bFireTime,
                    escorts[i].bPlannedFinalImpactTime,
                    escorts[i].firesAtBattleship ? "THREAT" : "NON-THREAT",
                    escorts[i].bShotsFired < escorts[i].bShotsPlanned
                        ? " | SOME SHOTS CANCELLED"
                        : ""
                );
                found = 1;
                break;
...
static void printBattleshipImpacts(void)
{
    ...
    for (i = 0; i < numberOfEscorts; i++)
    {
        ...
        {
            printf(
                "E%03d (%s) | B shots %d | Last impact %.4f s | E damage %.2f (%.0f%%) | %s\n",
                escorts[i].id,
                escorts[i].type->notation,
                escorts[i].bShotsFired,
                escorts[i].bImpactTime,
                escorts[i].damage,
                escorts[i].damage * 100.0,
...
)
{
        ...
        for (order = 1; order <= numberOfEscorts; order++)
        {
            for (i = 0; i < numberOfEscorts; i++)
            {
                if (escorts[i].attackOrder == order)
                {
                    fprintf(
                        file,
                        "%02d | E%03d | %s | Planned Shots %d | First Fire %.4f s | Final Impact %.4f s | %s%s\n",
                        order,
                        escorts[i].id,
                        escorts[i].type->notation,
                        escorts[i].bShotsPlanned,
    ...
    for (i = 0; i < numberOfEscorts; i++)
    {
        if (escorts[i].targetedByBattleship && escorts[i].bShotFired)
        {
            fprintf(
                file,
                "E%03d | %s | B Shots %d | Last Hit %.4f s | Damage %.4f | %s | Distance %.4f | Angle %.2f | Velocity %.4f\n",
                escorts[i].id,
                escorts[i].type->notation,
                escorts[i].bShotsFired,
                escorts[i].bImpactTime,
    ...
    fprintf(file, "B Shots Fired     : %d\n", battleshipShotsFired);
    fprintf(file, "B Current Impact  : %.8f\n",
            battleshipCurrentImpactPower);
...
)
{
    ...
    for (i = 0; i < numberOfEscorts; i++)
    {
        ...
        fprintf(file, " | E Shots Fired: %d",
                escorts[i].totalShotsFired);
        fprintf(file, " | E Damage: %.4f", escorts[i].damage);
        fprintf(file, " | Current Impact: %.8f",
                escorts[i].currentImpactPower);
    ...
    if (history != NULL)
    {
        ...
        if (cumulativeMode)
        {
            fprintf(
                history,
                "Damage Model      : %s\n",
                activeStage == STAGE_PART_2_C
                    ? "IP_n = IP_0 * exp(-gamma * n)"
                    : "Escort type impact power"
            );
...
static const char *activeStageName(void)
{
    if (activeStage == STAGE_PART_2_A)
        return "PART 2-A";
    if (activeStage == STAGE_PART_2_B)
...
static void runPart1BSimulation2(int cumulativeMode)
{
    ...
    char baselineMode[180];
    char jammedMode[180];
    char initialMode[180];
    char comparisonTitle[180];

    if (activeStage == STAGE_PART_1)
    {
        snprintf(baselineMode, sizeof(baselineMode), "%s",
                 cumulativeMode
                    ? "PART 1-C - PART 1-B SIMULATION 1 BASELINE"
                    : "PART 1-B - SIMULATION 1 BASELINE");
        snprintf(jammedMode, sizeof(jammedMode), "%s",
                 cumulativeMode
                    ? "PART 1-C - PART 1-B SIMULATION 2"
                    : "PART 1-B - SIMULATION 2");
        snprintf(initialMode, sizeof(initialMode), "%s",
                 cumulativeMode
                    ? "PART 1-C - PART 1-B SIMULATION 2 COMPARISON"
                    : "PART 1-B - SIMULATION 2 COMPARISON");
        snprintf(comparisonTitle, sizeof(comparisonTitle), "%s",
                 cumulativeMode
    ...
    {
        snprintf(baselineMode, sizeof(baselineMode), "%s - %s",
                 activeStageName(),
                 cumulativeMode
                    ? "PART 1-C / PART 1-B SIMULATION 1 BASELINE"
                    : "PART 1-B SIMULATION 1 BASELINE");
        snprintf(jammedMode, sizeof(jammedMode), "%s - %s",
                 activeStageName(),
                 cumulativeMode
                    ? "PART 1-C / PART 1-B SIMULATION 2"
                    : "PART 1-B SIMULATION 2");
        snprintf(initialMode, sizeof(initialMode), "%s - %s",
                 activeStageName(),
                 cumulativeMode
                    ? "PART 1-C / PART 1-B SIMULATION 2 COMPARISON"
                    : "PART 1-B SIMULATION 2 COMPARISON");
        snprintf(comparisonTitle, sizeof(comparisonTitle), "%s - %s",
                 activeStageName(),
                 cumulativeMode
                    ? "PART 1-C SIMULATION COMPARISON"
                    : "PART 1-B SIMULATION COMPARISON");
...
{
    printSection("SELECT PROJECT PART");

    printf("\n");
    printf("1. Part 1 - A, B and C\n");
    printf("2. Part 2 - A, B and C\n");
...
static int selectPart2Stage(void)
{
    printSection("SELECT PART 2 TASK");

    printf("\n");
    printf("1. Part 2-A - B reload time and attack order\n");
    printf("2. Part 2-B - Continuous Escort firing\n");
    printf("3. Part 2-C - Impact power degradation\n");
    printf("4. Return\n");

    return getInteger("\nEnter choice [1 - 4]: ", 1, 4);
}

static int selectScenario(int cumulativeOnly)
{
    printSection(cumulativeOnly
        ? "SELECT PART 1-C SIMULATION"
        : "SELECT PART 1 SIMULATION");
    ...
    if (!cumulativeOnly)
    {
        ...
        printf("4. Part 1-C - Redo Part 1-A\n");
        printf("5. Part 1-C - Redo Part 1-B Simulation 1\n");
        printf("6. Part 1-C - Redo Part 1-B Simulation 2\n");
        printf("7. Return\n");
...
void simulation(void)
{
    ...
    projectChoice = selectSimulationMode();
    ...
    if (projectChoice == 1)
    {
        activeStage = STAGE_PART_1;
        scenario = selectScenario(0);
    }
    else
    {
        part2Choice = selectPart2Stage();

        if (part2Choice == 4)
            return;

        activeStage = part2Choice;
        scenario = selectScenario(activeStage == STAGE_PART_2_C);
    }

    if (scenario == 7)
        return;

    cumulativeMode = scenario >= 4;
    ...
    battleshipReloadTime = 0.0;
    memset(escortReloadTimes, 0, sizeof(escortReloadTimes));
    memset(battleshipGammaValues, 0, sizeof(battleshipGammaValues));
    memset(escortGammaValues, 0, sizeof(escortGammaValues));
    ...
    if (activeStage == STAGE_PART_2_C)
    {
        printSection("IMPACT DEGRADATION GAMMA VALUES");

    printf("\nEnter a gamma value for every Battleship type.\n");

    {
        int typeIndex;
        const char battleshipNotations[4] = {'U', 'M', 'R', 'S'};
        ...
        for (typeIndex = 0; typeIndex < 4; typeIndex++)
        {
            ...
            snprintf(
                prompt,
                sizeof(prompt),
                "Enter Battleship %c gamma [%.3f - %.3f]: ",
                battleshipNotations[typeIndex],
                MIN_B_GAMMA,
                MAX_B_GAMMA
            );

            battleshipGammaValues[typeIndex] = getDouble(
                prompt,
                MIN_B_GAMMA,
                MAX_B_GAMMA
            );
        ...
        printf("\nEnter a gamma value for every Escort type.\n");

        {
            ...
            snprintf(
                prompt,
                sizeof(prompt),
                "Enter Escort %s gamma [%.3f - %.3f]: ",
                escortTypes[typeIndex].notation,
                MIN_E_GAMMA,
                MAX_E_GAMMA
            );

                escortGammaValues[typeIndex] = getDouble(
                    prompt,
                    MIN_E_GAMMA,
                    MAX_E_GAMMA
                );
    ...
    if (scenario == 1 || scenario == 4)
    {
        ...
        if (activeStage == STAGE_PART_1)
        {
            snprintf(
                ...
                sizeof(modeName),
                "%s",
                cumulativeMode ? "PART 1-C - PART 1-A" : "PART 1-A"
            );
        }
        else
        {
        ...
        runSinglePointScenario(modeName, cumulativeMode);
    }
    else if (scenario == 2 || scenario == 5)
    {
        runPart1BSimulation1(cumulativeMode);
    }
    else if (scenario == 3 || scenario == 6)
    {
        runPart1BSimulation2(cumulativeMode);
    }
}
